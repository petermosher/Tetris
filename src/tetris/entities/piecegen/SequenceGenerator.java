...
public class SequenceGenerator implements PieceGenerator {
  ...
  /*@ public model non_null List sequence;
      public constraint \old(sequence) == sequence;
      public constraint \old(sequence).equals(sequence);
      public constraint
        (\forall int i; 0 <= i && i < sequence.size();
         sequence.get(i) != null &&
         \typeof(sequence.get(i)) <: \type(Piece));
      private represents sequence = my_sequence;
      private constraint \old(my_sequence).equals(my_sequence);
      private constraint
        (\forall int i; 0 <= i && i < my_sequence.size();
         my_sequence.get(i) != null &&
         \typeof(sequence.get(i)) <: \type(Piece)); @*/
  ...
  /*@ in index;
      ...
      in index;
      public invariant 0 <= index;
      public invariant index < sequence.size();
      public constraint
        \old(index) < sequence.size() - 1 ==>
        index == \old(index) ||
        index == \old(index) + 1;
      public constraint
        \old(index) == sequence.size() - 1 ==>
          index == \old(index) ||
          index == 0;
      private represents index = my_index;
      private constraint
        \old(my_index) < my_sequence.size() - 1 ==>
        my_index == \old(my_index) ||
        my_index == \old(my_index) + 1;
      private constraint
        \old(my_index) == my_sequence.size() - 1 ==>
          my_index == \old(my_index) ||
          my_index == 0; @*/
  ...
  /*@ also public behavior
      assignable index, last_piece;
      ensures \result.equals((Piece) sequence.get(\old(index))); @*/
  /**
   * {@inheritDoc}
   */
