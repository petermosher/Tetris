...
public class RandomGenerator implements PieceGenerator {
  ...
  /*@ in random;
      public model non_null Random random;
      in random;
      private represents random = my_random; @*/
  ...
  /*@ public model non_null Point origin;
      public constraint \old(origin) == origin;
  ...
  /*@ public model non_null List piece_list;
      public constraint \old(piece_list) == piece_list;
      public constraint
        \old(piece_list.size()) == 0 ==> piece_list.size() == NUMBER_OF_PIECES;
      public invariant
        0 < piece_list.size() && piece_list.size() <= NUMBER_OF_PIECES;
      public invariant
        (\forall Piece p, q; piece_list.contains(p) && piece_list.contains(q);
         \typeof(p) != \typeof(q));
      public invariant
  ...
  /*@ also public behavior
      assignable random, last_piece, piece_list;
      ensures \result.origin.equals(origin);
      ensures \result == last_piece; @*/
  /**
   * {@inheritDoc}
   */
  ...
  /*@ private behavior
      requires my_piece_list.isEmpty();
      assignable piece_list;
