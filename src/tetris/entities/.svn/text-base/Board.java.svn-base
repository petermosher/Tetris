...
public class Board extends Observable {
  ...
  /*@ in width;
      ...
      in width;
      public invariant 0 < width;
      public constraint \old(width) == width;
      private represents width = my_width;
      private invariant 0 < my_width; @*/
  ...
  /*@ in row_list;
      public model non_null List row_list;
      in row_list;
      public invariant row_list.size() == height + ROWS_ABOVE_BOARD;
      public invariant (\forall int i; 0 <= i && i < row_list.size();
                        row_list.get(i) != null);
      private represents row_list = my_row_list;
      private invariant my_row_list.size() == height + ROWS_ABOVE_BOARD;
      private invariant (\forall int i; 0 <= i && i < row_list.size();
                         row_list.get(i) != null); @*/
  ...
  /*@ in piece_generator;
      public model non_null PieceGenerator piece_generator;
      in piece_generator;
      public constraint \old(piece_generator) == piece_generator;
      private represents piece_generator = my_piece_generator; @*/
  ...
  // @constraint All blocks in the current and next pieces have valid positions.
  /*@ in next_piece;
      public model non_null Piece next_piece;
      in next_piece;
      public invariant (\forall int i; 0 <= i && i < Piece.NUMBER_OF_BLOCKS;
                        isWithinBoard(next_piece.absolutePosition(i)));
      private represents next_piece = my_next_piece;
      private invariant (\forall int i; 0 <= i && i < Piece.NUMBER_OF_BLOCKS;
                         isWithinBoard(my_next_piece.absolutePosition(i))); @*/
  ...
  /*@ in last_lines_removed;
      public model int last_lines_removed;
      in last_lines_removed;
      public invariant 0 <= last_lines_removed &&
                       last_lines_removed <= Piece.NUMBER_OF_BLOCKS;
      private represents last_lines_removed = my_last_lines_removed;
      private invariant 0 <= my_last_lines_removed &&
                        my_last_lines_removed <= Piece.NUMBER_OF_BLOCKS; @*/
  ...
  //  0 or 4.
  /*@ in last_blocks_placed;
      public model int last_blocks_placed;
      in last_blocks_placed;
      public invariant last_blocks_placed == 0 ||
                       last_blocks_placed == Piece.NUMBER_OF_BLOCKS;
      private represents last_blocks_placed = my_last_blocks_placed;
      private invariant my_last_blocks_placed == 0 ||
  ...
  /*@ in changed;
      public model boolean changed;
      in changed;
      private represents changed = my_changed_flag; @*/
  ...
  /*@ in full;
      public model boolean full;
      in full;
      public constraint \old(full) ==> full;
      private represents full = my_full_flag;
      private constraint \old(my_full_flag) == my_full_flag; @*/
  ...
  /*@ requires
      (\forall int i; 0 <= i && i < the_sequence.size();
       ...
       \typeof(the_sequence.get(i)) <: \type(Piece)); @*/
  //@ ensures height == the_height;
  //@ ensures width == the_width;
  //@ ensures \fresh(current_piece);
  /*@ ensures
  ...
  /*@ ensures (\forall int i; 0 <= i && i < \result.length;
               Array.get(\result, i) == Array.get(row_list.get(the_y), i)); @*/
  /**
  ...
  //@ requires 0 <= the_y && the_y < height + ROWS_ABOVE_BOARD;
  /*@ ensures \result <==>
              (\forall int x; 0 <= x && x < width; rowAt(the_y)[x] == null) &&
              (\forall int i; 0 <= i && i < Piece.NUMBER_OF_BLOCKS;
  ...
  /*@ public behavior
      requires isWithinBoard(the_point);
      requires (\exists int i; 0 <= i && i < Piece.NUMBER_OF_BLOCKS;
                current_piece.absolutePosition(i).equals(the_point));
      ensures \result == current_piece.color; @*/
  /*@ also public behavior
      requires isWithinBoard(the_point);
      requires !(\exists int i; 0 <= i && i < Piece.NUMBER_OF_BLOCKS;
                current_piece.absolutePosition(i).equals(the_point));
      ensures \result == rowAt(the_point.y)[the_point.x]; @*/
  /**
  ...
  /*@ ensures
     \result <==>
     (\exists int b; 0 <= b && b < Piece.NUMBER_OF_BLOCKS;
      !isWithinBoard(the_piece.absolutePosition(b)) ||
      rowAt(the_piece.absolutePosition(b).y)
        [the_piece.absolutePosition(b).x] != null); @*/
  // the next postcondition is to help ESC/Java2
  /*@ ensures
      !\result ==>
      (\forall int b; 0 <= b && b < Piece.NUMBER_OF_BLOCKS;
       isWithinBoard(the_piece.absolutePosition(b))); @*/
  /**
  ...
  /*@ public behavior
      requires collides(current_piece.moveLeft());
      assignable last_lines_removed, last_blocks_placed, changed;
      ensures !changed;
      ensures current_piece == \old(current_piece);
      ensures last_lines_removed == 0;
      ensures last_blocks_placed == 0; @*/
  /*@ also public behavior
      requires !collides(current_piece.moveLeft());
      assignable current_piece, last_lines_removed, last_blocks_placed,
                 changed, _changed;
      ensures changed;
      ensures current_piece.equals(\old(current_piece.moveLeft()));
      ensures last_lines_removed == 0;
      ensures last_blocks_placed == 0; @*/
  /**
  ...
  // bottom of the board has no effect.
  /*@ public behavior
      requires collides(current_piece.moveRight());
      assignable last_lines_removed, last_blocks_placed, changed;
      ensures !changed;
      ensures current_piece == \old(current_piece);
      ensures last_lines_removed == 0;
  ...
  /*@ also public behavior
      ...
      assignable current_piece, last_lines_removed, last_blocks_placed,
                 changed, _changed;
      ensures changed;
      ensures current_piece.equals(\old(current_piece.moveRight()));
      ensures last_lines_removed == 0;
      ensures last_blocks_placed == 0; @*/
  /**
  ...
  /*@ public behavior
      requires !collides(current_piece.moveDown());
      assignable current_piece, last_lines_removed, last_blocks_placed,
                 changed, _changed;
      ensures current_piece.equals(\old(current_piece.moveDown()));
      ensures last_lines_removed == 0;
      ensures last_blocks_placed == 0;
      ensures changed; @*/
  ...
  /*@ also public behavior
      ...
      assignable current_piece, next_piece, last_lines_removed,
                 last_blocks_placed, changed, _changed, row_list, full;
      ensures last_lines_removed ==
        (\num_of int y; 0 <= y && y < height + ROWS_ABOVE_BOARD;
         \old(isRowFull(y)));
      ensures last_blocks_placed == Piece.NUMBER_OF_BLOCKS;
      ensures current_piece == \old(next_piece);
      ensures \fresh(next_piece);
      ensures next_piece == piece_generator.last_piece;
      ensures changed;
      ensures
        (\exists int y; height <= y && y < height + ROWS_ABOVE_BOARD;
         !isRowEmpty(y)) ==> full;
      ensures
        (\forall int y; 0 <= y && y < height + ROWS_ABOVE_BOARD;
         \old(isRowFull(y)) <==> !row_list.contains(\old(rowAt(y)))) &&
        (\forall int b, c;
         !\old(isRowFull(b)) && !\old(isRowFull(c)) && b < c;
         row_list.indexOf(\old(rowAt(b))) <
           row_list.indexOf(\old(rowAt(c))));
      ensures
        (\forall int y;
         height() + ROWS_ABOVE_BOARD - last_lines_removed <= y &&
           y < height() + ROWS_ABOVE_BOARD;
         isRowEmpty(y));
      ensures
        (\forall int b; 0 <= b && b < Piece.NUMBER_OF_BLOCKS;
         \old(current_piece.color.equals
  ...
  // bottom of the board has no effect.
  /*@ public behavior
      requires collides(current_piece.rotateClockwise());
      assignable last_lines_removed, last_blocks_placed, changed;
      ensures !changed;
      ensures current_piece == \old(current_piece);
      ensures last_lines_removed == 0;
  ...
  /*@ also public behavior
      ...
      assignable current_piece, last_lines_removed, last_blocks_placed,
                 changed, _changed;
      ensures changed;
      ensures current_piece.equals(\old(current_piece.rotateClockwise()));
      ensures last_lines_removed == 0;
      ensures last_blocks_placed == 0; @*/
  /**
  ...
  // bottom of the board has no effect.
  /*@ public behavior
      requires collides(current_piece.rotateCounterclockwise());
      assignable last_lines_removed, last_blocks_placed, changed;
      ensures !changed;
      ensures current_piece == \old(current_piece);
      ensures last_lines_removed == 0;
  ...
  /*@ also public behavior
      ...
      assignable current_piece, last_lines_removed, last_blocks_placed,
                 changed, _changed;
      ensures changed;
      ensures current_piece.equals
                (\old(current_piece.rotateCounterclockwise()));
      ensures last_lines_removed == 0;
      ensures last_blocks_placed == 0; @*/
  /**
  ...
  /*@ public behavior
      ...
      ensures last_lines_removed ==
        (\num_of int y; 0 <= y && y < height + ROWS_ABOVE_BOARD;
         \old(isRowFullUnderProjection(y)));
      ensures last_blocks_placed == Piece.NUMBER_OF_BLOCKS;
      ensures current_piece == \old(next_piece);
      ensures \fresh(next_piece);
      ensures next_piece == piece_generator.last_piece;
      ...
      ensures
        (\forall int y; 0 <= y && y < height + ROWS_ABOVE_BOARD;
         \old(isRowFullUnderProjection(y)) <==>
           !row_list.contains(\old(rowAt(y)))) &&
        (\forall int b, c;
         !\old(isRowFullUnderProjection(b)) &&
  ...
  /*@ private behavior
      assignable my_last_lines_removed, my_row_list; */
  /**
