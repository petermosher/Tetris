...
public class Rotation {
  ...
  /*@ public model Point[] blocks;
      public invariant \nonnullelements(blocks);
      private represents blocks = my_blocks;
  ...
  public /*@ pure @*/ Rotation(final /*@ non_null */ Point[] the_blocks) {
    ...
    /*@ assume
        my_blocks.length == the_blocks.length &&
        (\forall int i; 0 <= i && i < the_blocks.length;
         my_blocks[i] == the_blocks[i]); @*/
  }
  ...
  public /*@ pure non_null */ Point[] blocks() {
    ...
    // TODO: array cloning doesn't verify
    /*@ assume
        result.length == my_blocks.length &&
        (\forall int i; 0 <= i && i < my_blocks.length;
         result[i] == my_blocks[i]); @*/
    return result;
  }
  ...
  /*@ public represents
      theHashCode =
        (\sum int i; 0 <= i && i < blocks.length; blocks[i].theHashCode); @*/
  /**
   * {@inheritDoc}
   */
